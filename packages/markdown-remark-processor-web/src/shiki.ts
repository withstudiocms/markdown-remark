...
export async function createShikiHighlighter(
	...
	let theme = opts.theme ?? 'github-dark';
	const themes = opts.themes ?? {};
	const langAlias = opts.langAlias ?? {};
			...
			transformers: [
				{
					pre(node) {
						...
						const attributesClass = options?.attributes?.class;
						const attributesStyle = options?.attributes?.style;

						Object.assign(node.properties, options?.attributes);
	...
	return {
		codeToHast(code, lang, options = {}) {
			return highlight(code, lang || 'plaintext', options, 'hast') as Promise<Root>;
		},
