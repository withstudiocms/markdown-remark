...
export async function highlightCodeBlocks(tree: Root, highlighter: Highlighter): Promise<void> {
	...
	for (const node of nodes) {
		...
		const meta = (node.node.data as any)?.meta ?? node.node.properties.metastring ?? undefined;
		const code = toText(node.node, { whitespace: 'pre' });
		const result = await highlighter(code, node.language, { meta });
		...
		const index = node.grandParent.children.indexOf(node.parent);
		node.grandParent.children[index] = replacement;
	}
}
